...
Then(
  ...
  () => {
    ...
    cy.get(
      "[data-cy=table-list-mips] tr.maker-element-row:not(.maker-expanded-row) td.mat-column-status"
    ).each(($row) => {
...
When(
  "Types the tags collateral-onboarding, mip-set combined with AND in the search bar plus Enter",
  () => {
...
Then(
  "The found MIps should have the tags collateral-onboarding, mip-set",
  () => {
    const dataset = ["collateral-onboarding", "mip-set"];
