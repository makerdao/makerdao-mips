describe('Test MD Viewer', () => {
  ...
  it('should open box container', () => {
      ...
      cy.get('app-proposal-components .container .content div').each(($div,$idx)=>{
        cy.get('app-proposal-components .container .content div').eq($idx).click()
        cy.wait(2000)
        cy.location().then($loc=>{
          cy.get('app-proposal-components .container .content div').eq($idx).invoke('text').invoke('trim').should('contain',$loc.hash.substring(1))
          cy.visit('/mips/md-viewer?mdUrl=https:%2F%2Fraw.githubusercontent.com%2Fmakerdao%2Fmips%2Fmaster%2FMIP1%2FMIP1c4-Subproposal-Template.md')
       })
    })
